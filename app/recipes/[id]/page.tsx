            ...
            <div className="mb-6">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex gap-2">
                  {!macroAnalysis && !loadingAI && (
                    <Button
                      onClick={() => fetchAIAnalysis({ recipeName, servings, ingredients, mealType })}
                      variant="secondary"
                      size="sm"
              ...
              <div className="space-y-2">
                {ingredients.map((ing, index) => {
                  const ingredientRating = getIngredientRating(ing.ingredientName)
                  return (
                    <div key={index} className="flex items-center gap-2">
                      {ingredientRating && (
                        <div className="flex items-center" title={ingredientRating.reason}>
                          <div className={`w-3 h-3 rounded-full ${getTrafficLightClass(ingredientRating.rating)}`}></div>
                        </div>
                      )}
